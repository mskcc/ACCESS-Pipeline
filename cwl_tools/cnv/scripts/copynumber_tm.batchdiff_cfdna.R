...
# --
# 1. Use panel_A and panel_B targets
...
##to do: 1)need to incorporate patient sex from file manifests 2) general code clean up. 3) debug gene/probe FC discrepancies
...
source('/home/buehlere/access/v1/ACCESS-Pipeline/cwl_tools/cnv/scripts/textplot.R')
args <- commandArgs(trailingOnly=TRUE);
prefix <- args[1];
stdnormal_loess <- args[2];
targets_file = args[3]
covfile_loess = args[4]
doFull <- args[5];

#ACCESSv1-VAL-20180000 ACCESSv1-VAL-20180000_normal_ALL_intervalnomapqcoverage_loess.txt /dmp/hot/ptashkir/cfdna_scna/CNV_ACCESS/cBX_pipeline/data/scna_targets_coverage.txt ACCESSv1-VAL-20180000_tumor_ALL_intervalnomapqcoverage_loess.txt MIN
...
#remove all normals within the pool from the analysis
##normal.barcodes <- title[which(title$Class == "Normal"), ]["Barcode"]
##title <- title[-which(title$Barcode %in% normal.barcodes$Barcode), ]


tar = read.delim(targets_file,sep="\t",header=T,as.is=T,fill=T)
...
tiling_probe_ids <- gc_tiling[,'Interval'];


##stdn_cov <- read.delim(paste(stdnormal_loess,"_ALL_intervalnomapqcoverage_loess.txt",sep=""),sep="\t",header=T,as.is=T,fill=T);
stdn_cov <- read.delim(stdnormal_loess,sep="\t",header=T,as.is=T,fill=T);
...
# Remove PDPK1 from analysis
#idx <- which(gc[,'genes'] == 'PDPK1');
#if(length(idx) > 0){
#	gc <- gc[-idx,];
#	stdn_cov <- stdn_cov[-idx,];
#}
...
chr <- do.call('c',lapply(gc[,'Interval'],function(x){
	f <- unlist(strsplit(x,'\\:'));
	return(f[1]);
}));
...
tarG = cbind(tarG$Interval, tarG$Cyt, tarG$GeneExon)
gene.ann=tarG
gene.ann <- cbind(tar,'gene'=do.call('c',lapply(tar[,'GeneExon'],
            function(x){
			     	  if(x != ""){
			     	    f <- unlist(strsplit(x,'\\,'));
...
##gc.target <- as.character(tar[match(gc[,'Interval'],tar[,'Interval']),'Gene']);
...
d.stdn <- stdn_cov[,-c(1:3)];
colnames(d.stdn) <- paste('STDN_',colnames(d.stdn),sep="");
##index <- which(grepl('bc10',colnames(d.stdn)) & grepl('FFPE',colnames(d.stdn)));
##if(length(index)>0){
##	d.stdn <- d.stdn[,-index];
##}
...
# if exists >= 5 blood normals in pool, batch correct std blood normals to match current pool.
batch=FALSE #RP disable batch correction, for now
#if(sum(title$Class == 'Normal') >= 5){
if(batch==TRUE){
#pt.nm <- genelevel[,title[which(title[,'Class']=='Normal'),'Sample_ID']];
pt.nm <-d.stdn #gc
pt.nm.median <- apply(pt.nm,1,median);
autosome.mask <- rep(1,length(pt.nm.median));
autosome.mask[which(grepl('^X\\:',gc[,'Interval']) | grepl('^Y\\:',gc[,'Interval']))] <- 0;

d.stdn.bld.batch <- unlist(strsplit(colnames(d.stdn),'\\.'));
d.stdn.bld.batch <- d.stdn.bld.batch[grep('STDN',d.stdn.bld.batch)];
d.stdn.bld.median <- do.call('rbind',list(apply(d.stdn,1,function(row.i){
		return(tapply(row.i,d.stdn.bld.batch,median,na.rm=T));
	})));
d.stdn.bld.median <- t(d.stdn.bld.median);
rownames(d.stdn.bld.median) <- gc[,'Interval'];

batch.diff <- do.call('cbind',list(apply(d.stdn.bld.median,2,function(col.i){
ret <- pt.nm.median - col.i;
ret <- ret * autosome.mask;
return(ret);
})));

	d.stdn.bld.adj <- do.call('cbind',lapply(colnames(d.stdn),function(colname.i){
		batch <- unlist(strsplit(colname.i,'\\.'))[1];
		return(as.numeric(as.character(d.stdn[,colname.i]))+batch.diff[,batch]);
	}));
	colnames(d.stdn.bld.adj) <- colnames(d.stdn);
	d.stdn.bld <- d.stdn.bld.adj;

	#stdn.gender <- stdn_cov.title[match(colnames(d.stdn.bld),paste('STDN',stdn_cov.title[,'Barcode'],sep='_')),'Sample_type'];
	stdn.gender = d.stdn
	names(stdn.gender) <- colnames(d.stdn.bld);

  #gender <- do.call('c',lapply(title[match(colnames(pt.nm),title[,'Sample_ID']),'Gender'],function(x){
	#	if(x %in% c('Male','M','male')){
	#		return('M');
	#	}else{
	#		return('F');
	#	}
	#}));
	gender = 'F' #can implement parsing actual values / covg based methods later.

	Xsome.mask <- rep(0,nrow(d.stdn.bld));
	Xsome.mask[which(grepl('^X\\:',gc[,'Interval']))] <- 1;
	if(sum(gender == 'M') >= 5){
		d.stdn.bldM.median <- do.call('rbind',list(apply(d.stdn.bld[,which(stdn.gender == 'M')],1,function(row.i){
			return(tapply(row.i,d.stdn.bld.batch[which(stdn.gender == 'M')],median,na.rm=T));
		})));
		d.stdn.bldM.median <- t(d.stdn.bldM.median);
		rownames(d.stdn.bldM.median) <- gc[,'Interval'];

		pt.nm.median.M <- apply(pt.nm[,which(gender == 'M')],1,median);
		batch.diff.M <- do.call('cbind',list(apply(d.stdn.bldM.median,2,function(col.i){
			ret <- pt.nm.median.M - col.i;
			ret <- ret * Xsome.mask;
			return(ret);
		})));

		d.stdn.bld.adj <- do.call('cbind',lapply(colnames(d.stdn.bld),function(colname.i){
			if(stdn.gender[colname.i] != 'M'){
				return(as.numeric(as.character(d.stdn.bld[,colname.i])));
			}else{
				batch <- unlist(strsplit(colname.i,'\\.'))[1];
				return(as.numeric(as.character(d.stdn.bld[,colname.i]))+batch.diff.M[,batch]);
			}
		}));
		colnames(d.stdn.bld.adj) <- colnames(d.stdn.bld);
		d.stdn.bld <- d.stdn.bld.adj;
	}
	if(sum(gender == 'F') >= 5){
		d.stdn.bldF.median <- do.call('rbind',list(apply(d.stdn.bld[,which(stdn.gender == 'F')],1,function(row.i){
			return(tapply(row.i,d.stdn.bld.batch[which(stdn.gender == 'F')],median,na.rm=T));
		})));
		d.stdn.bldF.median <- t(d.stdn.bldF.median);
		rownames(d.stdn.bldF.median) <- gc[,'Interval'];

		pt.nm.median.F <- apply(pt.nm[,which(gender == 'F')],1,median);
		batch.diff.F <- do.call('cbind',list(apply(d.stdn.bldF.median,2,function(col.i){
			ret <- pt.nm.median.F - col.i;
			ret <- ret * Xsome.mask;
			return(ret);
		})));

		d.stdn.bld.adj <- do.call('cbind',lapply(colnames(d.stdn.bld),function(colname.i){
			if(stdn.gender[colname.i] != 'F'){
				return(as.numeric(as.character(d.stdn.bld[,colname.i])));
			}else{
				batch <- unlist(strsplit(colname.i,'\\.'))[1];
				return(as.numeric(as.character(d.stdn.bld[,colname.i]))+batch.diff.F[,batch]);
			}
		}));
		colnames(d.stdn.bld.adj) <- colnames(d.stdn.bld);
		d.stdn.bld <- d.stdn.bld.adj;
...
# batch correcting std ffpe normals to median-match in pool blood normals gets rid of the FFPE vs. Blood normal effect.
# do not correct std ffpe normals, just use nomapq values

# Compile post adjustment changes
#d.stdn <- cbind(d.stdn.bld,d.stdn.ffpe);

#index <- which(title$Class %in% c("Normal","PoolNormal")); #selects the normal barcodes
##if(length(index)>=1){
##	normal <- do.call('cbind',list(apply(genelevel[,title[index,'Sample_ID'],drop=F],2,as.numeric)));
##	colnames(normal) <- title[index,'Sample_ID'];
##	normal <- cbind(normal,d.stdn);
##}else{
normal <- d.stdn;
##}
...
normal.median <- apply(normal,2,median,na.rm=T);
normal <- do.call('cbind',lapply(seq(1,ncol(normal),1),function(i){
	col.i <- normal[,i];
	col.i[which(col.i < 0.1*normal.median[i])] <- NA;  # Difference from previous
	col.i[which(is.infinite(col.i) | is.nan(col.i))] <- NA;
	return(col.i);
}));
colnames(normal) <- names(normal.median);

low.cov.probes <- do.call('c',list(apply(normal[,grepl('STDN',colnames(normal))],1,function(cov){
	return(sum(is.na(cov))/length(cov));
})));
low.cov.probes <- cbind('Interval'=gc[,'Interval'],'pct_low'=low.cov.probes,tar[match(gc[,'Interval'],tar[,'Interval']),]);
low.cov.probes <- low.cov.probes[which(low.cov.probes[,'pct_low'] > 0.2 & !grepl('Y\\:',low.cov.probes[,'Interval'])),];
idx <- which(tar[,'Interval'] %in% low.cov.probes[,'Interval']);
normal[idx,] <- matrix(rep(NA,length(idx)*ncol(normal)),nrow=length(idx),ncol=ncol(normal));

y.probes <- gc[grep('^Y\\:',gc[,'Interval']),'Interval'];
y.probes <- y.probes[which(!(y.probes %in% c("Y:6111337-6111456","Y:6736141-6736260","Y:25310891-25311010","Y:9238599-9238718")))];
y.cov <- do.call('c',list(apply(normal[which(gc[,'Interval'] %in% y.probes),colnames(d.stdn)],2,function(col.i){
...
y.cov_grp <- abs(mean(y.cov, na.rm = T));

#Tumor
##index <- which(title$Class %in% c("Tumor")); #selects the tumor barcodes
tumor <- do.call('cbind',list(apply(gc[,-c(1:3),drop=F],2,function(x){
      ret <- as.numeric(x);
...
# plot T/N for matched samples
##patients <- unique(title[which(title[,'Class'] %in% c('Tumor','NormalGL')),'Patient_ID']);
##patients <- unique(title[which(title[,'Class'] %in% c('Tumor')),'Patient_ID']);
patients = colnames(d)
##patient.TN <- do.call('rbind',lapply(patients,function(pt.i){
##  tm.index <- which(title[,'Patient_ID'] == pt.i & title[,'Class'] %in% c('Tumor','NormalGL'));
##	tm.index <- which(title[,'Patient_ID'] == pt.i & title[,'Class'] %in% c('Tumor'));
##	tm <- title[tm.index,'Sample_ID'];
##	nm.index <- which(title[,'Patient_ID'] == pt.i & title[,'Class'] == 'Normal');
##	gender <- "-";
##	if(any(grepl('Gender',colnames(title)))){
##		#gender <- title[which(title[,'Patient_ID'] == pt.i & title[,'Class'] %in% c('Tumor','NormalGL')),'Gender'];
##		gender <- title[which(title[,'Patient_ID'] == pt.i & title[,'Class'] %in% c('Tumor')),'Gender'];
##		if(gender %in% c('Male','M','male')){
##			gender <- 'M';
##		}else{
##			gender <- 'F';
##		}
##	}
##	if(length(nm.index) == 1){
##    	    nm <- title[nm.index,'Sample_ID'];
##	}else{
##    	    if(length(nm.index) > 1){
##	    		nm <- title[nm.index[1],'Sample_ID'];
##    	    }else{
##    			nm <- NA;
##    	    }
##        }
##    	return(expand.grid('Patient'=pt.i,'Tumor'=tm,'Normal'=nm,'Gender'=gender));
##}));
##patient.TN <- cbind('Barcode'=title[match(patient.TN[,'Tumor'],title[,'Sample_ID']),'Barcode'],patient.TN);
...
seg.clus.p <- function(logratio, genelabel, make.plots=F,sample.id){
	...
	seg.probes <- do.call('c',lapply(names(logratio),function(targ.i){
		...
		if(nrow(X) == 0){
			...
			cat("Multiple seg\t",sample.id,"\t",targ.i,"\n");
	...
	sig.g <- abs(fc.genes) >= 2.0 & pg.adj < 0.05;
	...
	ret.intragenic <- do.call('rbind',lapply(all.genes,function(gene.i){
		...
		sig.x <- fc.x <= -2.0 & p.adj.x < 0.05;

		ann <- do.call('rbind',lapply(names(lr.x),function(targ.i){
			if(targ.i %in% tar[,'Interval']){
				return(tar[which(tar[,'Interval']==targ.i),c('Cyt','GeneExon')]);
			}else{
			  #print(targ.i)
		...
		if(length(lr.x) >= 3 & length(unique(lr.x)) > 2 & length(table(sig.x)) > 1){
			...
			if(ratio >= 3.7){
				...
				if((k2$size[clus.del] == 1 & clus.diff >= 1.0) |
				   ...
				   (k2$size[clus.del] >= 6 & clus.diff >= 0.5 & pct.stretch >= 0.5)){
				   	...
				   	lr.x <- logratio[probes];
				   	fc.x <- fc[names(genelabel)[which(flag)]];
				   	p.adj.x <- p.adj[names(genelabel)[which(flag)]];
				   	p.cluster <- rep(NA, length(lr.x));
				   	names(p.cluster) <- probes;
				   	p.cluster[names(cluster)] <- cluster;
				   	ann <- do.call('rbind',lapply(names(lr.x),function(targ.i){
						if(targ.i %in% gene.ann[,'Interval']){
							return(gene.ann[which(gene.ann[,'Interval']==targ.i),c('Cyt','GeneExon')]);
						}else{
							return(c('Cyt'=NA,'GeneExon'=NA));
						}
					}));
					ret <- cbind('table'=rep('Intragenic_loss',length(lr.x)),'lr'=lr.x,'NExons'=rep(1,length(lr.x)),
    	                        'chr'=do.call('c',lapply(names(lr.x),function(x){
									f <-  unlist(strsplit(x,'\\:'));
									return(f[1]);
			    				 })),'fc'=fc.x,'p.adj'=p.adj.x,'sig'=as.character(p.cluster),'sig.intragenic'=rep('-',length(lr.x)),ann);
			    	return(ret);
			    }
			}
		}
	...
	ann <- do.call('rbind',lapply(names(logratio),function(targ.i){
		if(targ.i %in% gene.ann[,'Interval']){
			return(gene.ann[which(gene.ann[,'Interval']==targ.i),c('Cyt','GeneExon')]);
		}else{  # (targ.i %in% gene.ann[,'Interval']){
			return(c('Cyt'=tar[which(tar[,'Interval']==targ.i),'Cyt'],'GeneExon'='Tiling'));
		}
	  #else{
	...
	if(!is.null(ret.intragenic)){
 	...
 	ret.probes <- cbind(ret.probes,'sig.intragenic'=sig.intragenic,ann);
        ...
        gene.cyt <- do.call('c',lapply(rownames(seg.genes),function(gene){
	        idx <- which(gene.ann[,'gene'] %in% gene);
		if(length(idx) == 0){
			return('-');
		}else{
			cyt <- gene.ann[idx,'Cyt'];
			cyt.table <- table(cyt);
...
cn.analysis <- function(tm.vec, auto_nm.vec, x_nm.vec, tm.label, genelabel,make.plot=F){
	# exclude Y
	...
	return(seg.clus.p(lr,genelabel,make.plots=make.plot,sample.id=tm.label));
}

if(doFull == "FULL"){
    out_file= paste(prefix,"_copynumber_segclusp.full.pdf", sep="");
}else{
    out_file= paste(prefix,"_copynumber_segclusp.pdf", sep="");
}
...
text(1, 8, "Copy number segmentation analysis",adj=0,cex=1.5,font=2)
##text(1,7.5, paste("Project pool: ", title$Pool[1],sep=""),adj=0,cex=1.5,font=2) #Add project name from title file data
text(1,7.5, paste("Project pool: ", prefix,sep=""),adj=0,cex=1.5,font=2)
text(1,7, paste("Baits version: ", prefix,sep=""),adj=0,cex=1.5,font=2)
##text(1,7, paste("Baits version: ", title$Bait_version[1],sep=""),adj=0,cex=1.5,font=2) #Add baits version from title file data


##all.normals <- colnames(normal);
all.normals = colnames(stdn_cov[,-c(1:3)])

##analysis.out <- do.call('rbind',lapply(seq(1,nrow(patient.TN),1),function(i){
analysis.out <- do.call('rbind',lapply(seq(1,length(patients),1),function(i){
	#pt <- as.character(patient.TN[i,'Patient']);
	#tm <- as.character(patient.TN[i,'Tumor']);
	#nm <- as.character(patient.TN[i,'Normal']);
	#bc <- as.character(patient.TN[i,'Barcode']);
	#gender <- as.character(patient.TN[i,'Gender']);
  pt=patients[i]
  tm=patients[i]
  nm = paste("STDN",all.normals[1], sep = "_") ###need to update this to be a matched normal
  gender = "-"  ###need to input this value from manifest

	#cat(pt,"\t",tm,"\n");
	tm.probes <- tumor[,tm];
	names(tm.probes) <- gc[,'Interval'];
	genelabel <- gc[,'genes'];
	names(genelabel) <- gc[,'Interval'];
	if(is.na(nm)){
	  nm.probes <- NA;
	}else{
  	nm.probes <- normal[,nm];
  		names(nm.probes) <- gc[,'Interval'];
	}


	cn.probes <- do.call('cbind',lapply(all.normals,function(nm){
	  nm = paste("STDN",nm, sep = "_")
	  nm.probes.i <- normal[,nm];
		ret.probes <- calc.fc(tm.probes,nm.probes.i);
		colnames(ret.probes) <- paste(nm,"|",colnames(ret.probes),sep="");
		return(ret.probes);
	}));
	cn.probes <- cbind(gc[,1:3],cn.probes);

	# Best normal analysis - v2
	# - use lr for assessing noise
	# - use autosomal probes
	# - use results from calc.fc
	cn.probes.autosomes <- cn.probes[which(!grepl('^X\\:',cn.probes[,'Interval']) & !grepl('^Y\\:',cn.probes[,'Interval'])),];
	...
	cn.probes.X <- cn.probes[which(grepl('^X\\:',cn.probes[,'Interval'])),grep('lr',colnames(cn.probes)),drop=F];
	colnames(cn.probes.X) <- gsub('\\|lr','',colnames(cn.probes.X));
#	cn.probes.X <- cn.probes.X[,which(colnames(cn.probes.X) %in% c(nm,paste('STDN_',stdn_cov.title[which(stdn_cov.title[,'Sample_type'] == pt.gender),'Barcode'],sep='')))];
	...
	best.nm.probes.X <- normal[,best.norm.X];
	...
	if(doFull == 'FULL'){
        ...
        sig.list <- cn.analysis(tm.probes, best.nm.probes.auto, best.nm.probes.X, tm, genelabel,make.plot=F);
    }
	...
	sig.probes <- cbind('genes'=tar[match(rownames(sig.probes),tar[,'Interval']),'Gene'],
	...
	lr.plot_tiling <- lr;
	lr.plot_exon <- lr;
	lr.plot_tiling[-which(cn.probes[,'Interval'] %in% tiling_probe_ids)] <- NA;
	lr.plot_exon[which(cn.probes[,'Interval'] %in% tiling_probe_ids)] <- NA;
	plot(lr.plot_tiling,xlab="", ylab="",main="",xlim=c(0,nrow(normal)),ylim=c(-a,a),xaxt="n",pch=20,col="brown",mgp=c(2,1,0),mar=c(3,5,2,5));
	par(new=T);
	plot(lr.plot_exon,xlab="Targeted exons per chromosome", ylab="LOG2 Tumor/Normal Ratio",main=ratiolabel,xlim=c(0,nrow(normal)),ylim=c(-a,a),xaxt="n",pch=20,col="navy",mgp=c(2,1,0),mar=c(3,5,2,5))
	par(new=F);
	abline(v=linepos,lty=3);
	abline(h=-1,lty=2);
	abline(h=1,lty=2);
	...
	idx <- which(!grepl('_',sig.gene.gains[,'genes']) & as.numeric(sig.gene.gains[,'fc']) >= 2.0 & as.numeric(sig.gene.gains[,'p.adj']) < 0.05);
	...
	idx <- which(!grepl('_',sig.gene.losses[,'genes']) & as.numeric(sig.gene.losses[,'fc']) <= -2.0 & as.numeric(sig.gene.losses[,'p.adj']) < 0.05);
	...
	tiling[grep('_',sig.probes[,'GeneExon'])] <- 'Tiling';
	gene.refseq.exon.boundaries <- do.call('rbind',lapply(as.character(sig.probes[,'GeneExon']),function(gene_exon){
	  	...
	  	if(grepl('_',gene_exon) | gene_exon == ""){
	...
	#sink(paste(tm,"_",bc,"_",prefix,"_copynumber.json",sep=""));
	#probe.json <- list('region'=rownames(sig.probes),
	#  					 'x'=seq(1,nrow(sig.probes),1),
	#  					 'lr'=as.numeric(as.character(sig.probes[,'lr'])),
	#  					 'pval'=as.numeric(as.character(sig.probes[,'p.adj'])),
	#  					 'sig'=as.numeric(as.character(sig.probes[,'sig'])),
	#  					 'sig.intragenic'=as.numeric(as.character(sig.probes[,'sig.intragenic'])),
	#  					 'fc'=as.numeric(as.character(sig.probes[,'fc'])),
	#  					 'chr'=as.character(sig.probes[,'fc']),
	#  					 'gene'=as.character(gene.refseq.exon.boundaries[,'Gene']),
	#					 'txID'=as.character(gene.refseq.exon.boundaries[,'Refseq']),
	#					 'exon'=as.character(gene.refseq.exon.boundaries[,'Exon']),
	#					 'boundary'=as.character(gene.refseq.exon.boundaries[,'Boundaries']),
	#					 'Cyt'=as.character(sig.probes[,'Cyt']),
	#					 'type'=tiling);
	#cat(toJSON(probe.json));
	#sink();

	ret <- cbind('sample'=rep(tm,nrow(sig.list)),
