...
PLASMA_SAMPLE_TYPE = re.compile(r"(^TP|^TB|^NP)")
# PLASMA_SAMPLE_TYPE = ["TP", "NP", "T"]
BUFFY_SAMPLE_TYPE = re.compile(r"(^N)")
...
EXPECTED_BAIT_VERSION = {"v1", "v2"}
ASSAY_NAME = "ACCESS"
PROJECT_NAME = re.compile("^" + ASSAY_NAME + "[HL]{0,1}v[0-9]-[A-Za-z]+-[0-9]{8}.*")
BAIT_SEARCH = re.compile("^" + ASSAY_NAME + "[HL]{0,1}v[0-9]")
MERGED_LANE_VALUE = "0"
