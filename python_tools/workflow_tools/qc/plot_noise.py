...
def noise_by_substitution_plot(noise_by_substitution_table):
    ...
    six_class_noise_by_substitution = pd.DataFrame(
        columns=["Sample", "Class", "AltPercent"]
    )
    for sample in all_samples:
        for class_pair in substitution_classes:
            ...
            substitution_class_boolv = noise_by_substitution_table["Substitution"].isin(
                original_classes
            )
            sample_boolv = noise_by_substitution_table[SAMPLE_ID_COLUMN] == sample
            subset = noise_by_substitution_table[
                substitution_class_boolv & sample_boolv
            ]
            combined_class_altcount = float(subset["AltCount"].sum())
            combined_class_genotype_count = float(subset["GenotypeCount"].sum())
            class_noise = combined_class_altcount / (
                combined_class_altcount + combined_class_genotype_count + EPSILON
            )

            # Change from fraction to percent
            class_noise = class_noise * 100.0
            new = pd.DataFrame(
                {"Sample": [sample], "Class": final_label, "AltPercent": class_noise}
            )
            six_class_noise_by_substitution = six_class_noise_by_substitution.append(
                new
            )
    ...
    g = sns.FacetGrid(
        six_class_noise_by_substitution, col="Sample", col_wrap=num_cols, sharey=True
    )
    ...
    six_class_noise_by_substitution.to_csv(
        "noise_by_substitution.tsv", sep="\t", index=False
    )
...
def main():
    ...
    noise_by_substitution_table = read_df(
        args.noise_by_substitution, header="infer"
    ).fillna(0)
    ...
    noise_by_substitution_table = noise_by_substitution_table[
        noise_by_substitution_table["Method"] == "Total"
    ]

    # Cleanup sample IDs (in Noise table as well as Title File)
    sample_ids = title_file[SAMPLE_ID_COLUMN].tolist()
    noise_table[SAMPLE_ID_COLUMN] = noise_table[SAMPLE_ID_COLUMN].apply(
        extract_sample_name, args=(sample_ids,)
    )
    noise_by_substitution_table[SAMPLE_ID_COLUMN] = noise_by_substitution_table[
        SAMPLE_ID_COLUMN
    ].apply(extract_sample_name, args=(sample_ids,))

    # Merge noise with title file
    noise_and_title_file = noise_table.merge(title_file, on=SAMPLE_ID_COLUMN)
    noise_by_substitution_table = noise_by_substitution_table.merge(
        title_file, on=SAMPLE_ID_COLUMN
    )

    # Filter to just Plasma samples (if there are any)
    plasma_samples = noise_and_title_file[
        noise_and_title_file[TITLE_FILE__SAMPLE_TYPE_COLUMN] == "Plasma"
    ][SAMPLE_ID_COLUMN]
    plasma_noise_by_substitution = noise_by_substitution_table[
        noise_by_substitution_table[TITLE_FILE__SAMPLE_TYPE_COLUMN] == "Plasma"
    ][SAMPLE_ID_COLUMN]
    if not len(plasma_samples) == 0:
        ...
        noise_by_substitution_boolv = noise_by_substitution_table[
            SAMPLE_ID_COLUMN
        ].isin(plasma_noise_by_substitution)
        noise_by_substitution_table = noise_by_substitution_table.loc[
            noise_by_substitution_boolv
        ]

    # Sort in same order as R code (by sample class)
    # Use a stable mergesort instead of quicksort default
    noise_and_title_file = noise_and_title_file.sort_values(
        TITLE_FILE__SAMPLE_CLASS_COLUMN, kind="mergesort"
    ).reset_index(drop=True)
    noise_by_substitution_table = noise_by_substitution_table.sort_values(
        TITLE_FILE__SAMPLE_CLASS_COLUMN, kind="mergesort"
    ).reset_index(drop=True)
