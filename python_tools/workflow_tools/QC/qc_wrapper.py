...
def cleanup_sample_names(table, sample_names):
    ...
    sample_id_search = '.*(' + sample_id_search + ').*'
    table[SAMPLE_ID_COLUMN] = table[SAMPLE_ID_COLUMN].str.replace({sample_id_search, r'\1'}, regex=True)
...
def subset_and_save_file(filename, sample_names, waltz_src, waltz_dst):
    ...
    boolv = df['ID'].astype(object).str.contains(sample_search, na=False)
...
def run_qc_for_lane(title_file, lane, args):
    ...
    for waltz_src_name, waltz_src_loc in folder_pairs:
        waltz_dst = RESULTS_DIR + waltz_src_name
    ...
    std_waltz_loc = BASE + RESULTS_DIR + 'standard_waltz'
