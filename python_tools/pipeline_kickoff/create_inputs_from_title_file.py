...
def load_fastqs(data_dir):
    ...
    folders_2 = filter(
        lambda folder: any([FASTQ_1_FILE_SEARCH in x for x in folder[2]]), folders
    )
    folders_3 = filter(
...
def perform_duplicate_barcodes_check(title_file):
    ...
    for lane in title_file[TITLE_FILE__LANE_COLUMN].unique():
        ...
        if np.sum(lane_subset[TITLE_FILE__BARCODE_INDEX_1_COLUMN].duplicated()) > 0:
            raise Exception(
                DELIMITER
                + "Duplicate barcodes for barcode 1, lane {}. Exiting.".format(lane)
            )

        if np.sum(lane_subset[TITLE_FILE__BARCODE_INDEX_2_COLUMN].duplicated()) > 0:
            raise Exception(
                DELIMITER
                + "Duplicate barcodes for barcode 2, lane {}. Exiting.".format(lane)
            )
...
def remove_missing_samples_from_title_file(title_file, fastq1, title_file_path):
    ...
    if samples_not_found.shape[0] > 0:
        print(
            DELIMITER
            + "Error: The following samples were missing either a read 1 fastq, read 2 fastq, or sample sheet. "
            + "These samples will be removed from the title file so that the remaining samples can be run."
        )
        print(
            "Please perform a manual check on the inputs file before running the pipeline."
        )
...
def check_i5_index(title_file_i5, sample_sheet_i5):
    ...
    err_string = "i5 index from title file {} does not match i5 index from SampleSheet {}. Aborting.".format(
        title_file_i5, sample_sheet_i5
    )

    assert (
        i5_matches_non_reverse_complemented or i5_matches_reverse_complemented
    ), err_string
...
def perform_barcode_index_checks_i5(title_file, sample_sheets):
    ...
    for sample_id in title_file[TITLE_FILE__SAMPLE_ID_COLUMN]:
        ...
        matching_sample_sheets = [
            ...
            if sample_id + SAMPLE_SEP_DIR_DELIMETER in s.get("path")
        ]
        assert (
            len(matching_sample_sheets) == 1
        ), "Incorrect matching sample sheet count for sample {}".format(sample_id)
        ...
        try:
            ...
            print(
                "Index2 not found in SampleSheet.csv. Skipping i5 barcode index validation."
            )
            return
    ...
    all_non_reverse_complemented = all(
        [match_type == NON_REVERSE_COMPLEMENTED for match_type in i5_sequencer_types]
    )
    all_reverse_complemented = all(
        [match_type == REVERSE_COMPLEMENTED for match_type in i5_sequencer_types]
    )

    assert (
        all_non_reverse_complemented or all_reverse_complemented
    ), "Not all barcodes followed same i5 index scheme"

    if all_non_reverse_complemented:
        print(
            DELIMITER
            + "All i5 barcodes match without reverse-complementing, sequencer was one of the following:"
        )
        print("NovaSeq\nMiSeq\nHiSeq2500")
        ...
        print(
            DELIMITER
            + "All i5 barcodes match after reverse-complementing, sequencer was one of the following:"
        )
        print("HiSeq4000\nMiniSeq\nNextSeq")
...
def include_fastqs_params(fh, data_dir, title_file, title_file_path, force):
    ...
    fastq1, fastq2, sample_sheets = sort_fastqs(
        fastq1, fastq2, sample_sheets, title_file
    )
    ...
    for key in samples_info:
        samples_info[key] = [
            x.strip() if type(x) == str else x for x in samples_info[key]
        ]
...
def perform_length_checks(fastq1, fastq2, sample_sheet, title_file):
    ...
    try:
        ...
        print(
            DELIMITER
            + "Error: Different number of read 1 and read 2 fastqs: {}".format(repr(e))
        )
    ...
    try:
        ...
        print(
            DELIMITER
            + "Error: Different number of sample sheet files & read 1 fastqs: {}".format(
                repr(e)
            )
        )
        print("fastq1: {}".format(len(fastq1)))
    ...
    try:
        ...
        print(
            DELIMITER
            + "Error: Different number of fastqs files and samples in title file: {}".format(
                repr(e)
            )
        )
        print("fastq1: {}".format(len(fastq1)))
...
def write_inputs_file(args, title_file, output_file_name):
    ...
    title_file_obj = {
        "title_file": {"class": "File", "path": os.path.abspath(args.title_file_path)}
    }
    ...
    inputs_yaml_object = {
        "inputs_yaml": {"class": "File", "path": os.path.abspath(output_file_name)}
    }
...
def check_final_file(output_file_name):
    ...
    try:
        ...
        print(
            DELIMITER
            + "It looks like there aren't enough entries for one of these fields: {}".format(
                fields_per_sample
            )
        )
        print(
            "Most likely, one of the samples is missing a read 1 fastq, read 2 fastq and/or sample sheet"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "-d", "--data_dir", help="Directory with fastqs and samplesheets", required=True
    )
    parser.add_argument(
        "-p",
        "--project_name",
    ...
    parser.add_argument(
        "-o", "--output_file_name", help="Name of yaml file for pipeline", required=True
    )
    parser.add_argument(
        "-f", "--force", help="Skip validation", required=False, action="store_true"
    )
...
def perform_validation(title_file, title_file_path, project_name):
    ...
    if not project_name in title_file_path:
        print(
            "WARNING: project ID not found in title file path. Are you sure you are using the correct title file?"
        )
    ...
    for lane in title_file[TITLE_FILE__LANE_COLUMN].unique():
        ...
        if np.sum(lane_subset[TITLE_FILE__BARCODE_ID_COLUMN].duplicated()) > 0:
            raise Exception(
                DELIMITER + "Duplicate barcode IDs in lane {}. Exiting.".format(lane)
            )

    if np.sum(
        title_file[TITLE_FILE__SAMPLE_CLASS_COLUMN].isin(ALLOWED_SAMPLE_DESCRIPTION)
    ) < len(title_file):
        raise Exception(
            DELIMITER
            + "Not all sample classes are in {}".format(
                ",".join(ALLOWED_SAMPLE_DESCRIPTION)
            )
        )

    if np.sum(
        title_file[TITLE_FILE__SAMPLE_TYPE_COLUMN].isin(ALLOWED_SAMPLE_TYPE_DESCRIPTION)
    ) < len(title_file):
        raise Exception(
            DELIMITER
            + "Not all sample types are in {}".format(
                ",".join(ALLOWED_SAMPLE_TYPE_DESCRIPTION)
            )
        )
    ...
    if title_file[TITLE_FILE__LANE_COLUMN].isnull().any():
        raise Exception(
            DELIMITER + "Lane column of title file must not be missing or NA"
        )
...
def print_user_message():
    print(DELIMITER)
    print(
        "You've just created the inputs file. Please double check its entries before kicking off a run."
    )
    print("Common mistakes include:")
    print(
        "1. Trying to use test parameters on a real run (accidentally using the -t param)"
    )
    print("2. Using the wrong bedfile for the capture")
    print(
        "3. Working in the wrong virtual environment (are you sure you ran setup.py install?)"
    )
    print(
        "4. Not specifying the correct parameters for logLevel or cleanWorkDir "
        + "(if you want to see the actual commands passed to the tools, or keep the temp outputs after a successful run)"
    )
    print(
        "5. Did you source the workspace_init.sh script (to make sure the PATH variable is set correctly?)"
    )
    print(
        '6. The "Sex" column of the title file will only correctly idenfity patients with [Male, M, Female, F] entries (although other entries will still be accepted).'
    )
